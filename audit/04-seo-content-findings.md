...
| Plugin versions | **Partly resolved.** `audit/05-architecture-and-code-quality.md` §2 re-inventoried the platform: WordPress 7.0.4 is current, Yoast 28.1 is current, Beaver Builder is 2.10.3.1, PowerPack is 2.40.1.6. **Ultimate Addons' version still cannot be determined from the frontend** — its only asset carries the WordPress core version as its `?ver=`, which is the fallback for a script enqueued with no version argument, not the plugin's version. The "2.10.3" in the original inventory was a misread of a `?ver=` string. Re-inventory from wp-admin before assessing the `≤1.5.9` CVE range. |
