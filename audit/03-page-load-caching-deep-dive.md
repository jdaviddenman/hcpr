...
| `rw-embed-data.s3.amazonaws.com` | ReviewWave config JS | **not verifiable** — endpoint returns 403 to direct requests | Lighthouse reports 55.3 KiB transferred, uncompressed | **YES.** The single largest render-blocking resource at 1,570 ms, and the only entry in Lighthouse's text-compression audit (36.6 KiB of available savings). Third-party S3 metadata is not fixable from this site — the fix is to `defer` the script so it stops blocking first paint. |
